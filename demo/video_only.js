      ...
      ws.on('message', function incoming(message) {
        ...
        switch (msg.type) {
                    ...
                    channel.request("transport.setMaxBitrate", { "routerId": 111111, "transportId": 222222 }, { bitrate: 1200000 })
                      .then((data) => {
                        channel.request("transport.setRemoteDtlsParameters", { "routerId": 111111, "transportId": 222222 }, dtlsdata)
                          .then((data) => {
                            if (!pubData.video.rtx.payloadType) {
                              videodata.rtpParameters.codecs = [videodata.rtpParameters.codecs[0]];
                              delete videodata.rtpParameters.encodings[0].rtx;
                              videodata.rtpMapping.codecPayloadTypes = [videodata.rtpMapping.codecPayloadTypes[0]];
                            }
                            channel.request("router.createProducer", { "routerId": 111111, "transportId": 222222, "producerId": 333333 }, videodata)
                              .then((data) => {
                                var pubsdp = {
                                  op: 'pub',
                                  type: 'answer',
          ...
          case 'sub1':
            ...
            channel.request("router.createConsumer", { "routerId": 111111, "producerId": 333333, "consumerId": 444444 }, { kind: 'video', pubVideoCodec: pubData.video.payloadType, subVideoCodec: subData.video.payloadType, pubRtxCodec: pubData.video.rtx.payloadType, subRtxCodec: subData.video.rtx.payloadType })
              .then((data) => {
                // channel.request("router.createWebRtcTransport", { "routerId": 111111, "transportId": 555555 }, { "tcp": false, ipv4: '192.168.199.111' })
                // channel.request("router.createWebRtcTransport", { "routerId": 111111, "transportId": 555555 }, { "tcp": false, ipv4: '100.100.81.210' })
                  ...
                  .then((data) => {
                    ...
                    channel.request("transport.setRemoteDtlsParameters", { "routerId": 111111, "transportId": 555555 }, dtlsdata)
                      .then((data) => {
                        channel.request("consumer.enable", { "routerId": 111111, "transportId": 555555, "producerId": 333333, "consumerId": 444444 }, enablevideo)
                          .then((data) => {
                            var subsdp = {
                              op: 'sub1',
                              type: 'answer',
          ...
          case 'sub2':
            ...
            channel.request("router.createConsumer", { "routerId": 111111, "producerId": 333333, "consumerId": 666666 }, { kind: 'video', pubVideoCodec: pubData.video.payloadType, subVideoCodec: subData.video.payloadType, pubRtxCodec: pubData.video.rtx.payloadType, subRtxCodec: subData.video.rtx.payloadType })
              .then((data) => {
                                // channel.request("router.createWebRtcTransport", { "routerId": 111111, "transportId": 777777 }, { "tcp": false, ipv4: '192.168.199.112' })
                                // channel.request("router.createWebRtcTransport", { "routerId": 111111, "transportId": 777777 }, { "tcp": false, ipv4: '100.100.81.210' })
                  ...
                  .then((data) => {
                    ...
                    channel.request("transport.setRemoteDtlsParameters", { "routerId": 111111, "transportId": 777777 }, dtlsdata)
                      .then((data) => {
                        channel.request("consumer.enable", { "routerId": 111111, "transportId": 777777, "producerId": 333333, "consumerId": 666666 }, enablevideo)
                          .then((data) => {
                            var subsdp = {
                              op: 'sub2',
                              type: 'answer',
