...
namespace RTC
{
	...
	std::map<std::string, struct sockaddr_storage> UdpSocket::sockaddrStorageIPv4s;
	std::map<std::string, struct sockaddr_storage> UdpSocket::sockaddrStorageIPv6s;
	...
	void UdpSocket::ClassInit()
	{
		...
		if (Settings::configuration.rtcIPv4s.size() > 0)
		{
			     ...
			     it++)
			{
				...
				err = uv_ip4_addr(it->first.c_str(), 0, reinterpret_cast<struct sockaddr_in*>(&xxxx));

				if (err != 0)
					MS_THROW_ERROR("uv_ipv4_addr() failed: %s", uv_strerror(err));

				RTC::UdpSocket::sockaddrStorageIPv4s[it->first] = xxxx;
			}
		}

		if (Settings::configuration.rtcIPv6s.size() > 0)
		{
			...
			{
				struct sockaddr_storage xxxx;
				err = uv_ip6_addr(it->first.c_str(), 0, reinterpret_cast<struct sockaddr_in6*>(&xxxx));

				if (err != 0)
					MS_THROW_ERROR("uv_ipv6_addr() failed: %s", uv_strerror(err));
					
				RTC::UdpSocket::sockaddrStorageIPv6s[it->first] = xxxx;
			}
		}
	...
	uv_udp_t* UdpSocket::GetRandomPort(int addressFamily, std::string ip)
	{
		MS_TRACE();

		if (addressFamily == AF_INET && !Settings::configuration.rtcIPv4s[ip])
			MS_THROW_ERROR("IPv4 family not available for RTC");
		else if (addressFamily == AF_INET6 && !Settings::configuration.rtcIPv6s[ip])
		...
		switch (addressFamily)
		{
			case AF_INET:
				...
				bindAddr       = RTC::UdpSocket::sockaddrStorageIPv4s[ip];
				listenIp       = ip.c_str();
				break;

			case AF_INET6:
				availablePorts = &RTC::UdpSocket::availableIPv6Ports;
				bindAddr       = RTC::UdpSocket::sockaddrStorageIPv6s[ip];
				listenIp       = ip.c_str();
	...
	UdpSocket::UdpSocket(Listener* listener, int addressFamily, std::string ip)
