...
namespace RTC
{
	...
	uint32_t Producer::ReadBits(int32_t n)
	{
		int r = 0;
		int i;
		for (i = 0; i < n; i++)
	...
	uint32_t Producer::ReadExponentialGolombCode()
	{
		int r = 0;
		int i = 0;
	...
	void Producer::Parse(uint8_t* pStart, uint16_t nLen)
	{
		m_pStart      = pStart;
		m_nLength     = nLen;
		m_nCurrentBit = 0;
		...
		if (
		  profile_idc == 100 || profile_idc == 110 || profile_idc == 122 || profile_idc == 244 ||
		...
		int Width = ((pic_width_in_mbs_minus1 + 1) * 16) - frame_crop_right_offset * 2 -
		             frame_crop_left_offset * 2;
		int Height = ((2 - frame_mbs_only_flag) * (pic_height_in_map_units_minus1 + 1) * 16) -
		              (frame_crop_bottom_offset * 2) - (frame_crop_top_offset * 2);

		this->transport->width  = Width;
		this->transport->height = Height;
	}
	...
	void Producer::ReceiveRtpPacket(RTC::RtpPacket* packet)
	{
		...
		if (packet->GetPayloadType() == this->pubVideoCodec)
		{
			switch (Utils::Byte::Get1Byte(packet->GetPayload(), 0) & 0x1f)
			{
				case 7:
					Parse(packet->GetPayload() + 1, packet->GetPayloadLength() - 1);
					break;
				case 24:
					auto l = Utils::Byte::Get2Bytes(packet->GetPayload(), 1);
					auto t = Utils::Byte::Get1Byte(packet->GetPayload(), 3) & 0x1f;
					if (t == 7)
					{
						Parse(packet->GetPayload() + 4, l - 1);
					}
					break;
			}
		}
